...
2. 点击 **生成计划** → 等待 1-2 秒
3. 看：意图解析面板（场景 / 人数 / 出发 / 时长 / 最远距离）
4. 看：推荐计划卡片（综合评分、理由、风险、预估费用）
5. 看：时间线表格（时间 / 步骤 / 地点 / 时长）
6. 展开 **工具调用追踪**：查看每个 tool 的状态、摘要、耗时
7. 点击 **确认并执行**
8. 看：执行结果（已购票 / 已预约，含 booking_id）
9. 看：分享消息（点击右上角图标一键复制）
...
```bash
pytest tests/ -v
# 期望：43 passed in ~1s（无 OPENAI_API_KEY 亦可通过）
```
