...
```bash
python -m src.cli.main 明天早上带孩子去动物园
python -m src.cli.main 今天晚上带孩子出去玩
python -m src.cli.main 待会和老婆去吃烛光晚餐
```
...
# export NATIVE_PLANNING_API_URL=http://localhost:8000  # macOS/Linux
streamlit run src/ui/app.py
```
...
# 期望：321 passed（无 OPENAI_API_KEY 亦可通过）
```
...
> **重要：** 确保 `.env` 文件已配置 `OPENAI_API_KEY`，否则意图解析会显示 `[rule-based]`。
